...
#                 print(f"Try move ({i},{j}) => score = {score}")

#                 if score > bestScore:
#                     bestScore = score
#                     move = (i, j)

#     print(f"\nAI chooses: {move} with score {bestScore}\n")
...
if __name__ == "__main__":#It is ignored if you do: import rulebased
    ...
    while not winned:
        ...
        clm = int(input("Enter col: "))

        if row < 0 or row >= 5 or clm < 0 or clm >= 5 or board[row][clm] != ' ':
            print("Invalid move")
            continue
        ...
        #bestMove(board, depth=4)
        row, clm = minimax_agent(board, 'X')
        board[row][clm] = 'X'
    ...
    if checkWinner(board) == 1:
        print("Tie!\n you can play another game and know the smartest")
    else:
